...
- 圖表輸出：
  ...
  - Model Comparison (Accuracy / Recall / Precision)
...
1. 辨識高價值顧客族群（如年齡較高、網站停留時間長、會員身份），以更有效率投放資源
2. 驗證網站停留時間對購買的顯著影響，提供 UX 團隊優化方向
3. 提供會員計畫的量化成效，作為擴展會員策略依據
4. 建立資料驅動的轉換率優化基礎模型，能快速調整行銷活動與預算
...
## 所有生成的圖表
皆已保存在 `outputs/` 資料夾，並在此 README 中直接嵌入以利快速檢視。
